...
switch ($opcion) {
    case 1:
        ...
        $consulta = "INSERT INTO w_pagoprovi 
        (folio_provi,fecha_pagoprovi,referencia_pagoprovi,observaciones_pagoprovi,metodo_pagoprovi,monto_pagoprovi,usuario) 
        VALUES ('$foliocxp','$fechavp','$referenciavp','$observacionesvp','$metodovp','$montovp','$usuario')";
        $resultado = $conexion->prepare($consulta);

        if ($resultado->execute()) {

            $consulta = "UPDATE w_provision SET saldo_provi='$saldofin' where folio_provi='$foliocxp'";
            $resultado = $conexion->prepare($consulta);
            if ($resultado->execute()) {
                $res = 1;
            }
    ...
    case 2:
/*
        $consulta = "INSERT INTO w_pagoprovi (folio_cxp,fecha_pagocxp,referencia_pagocxp,observaciones_pagocxp,metodo_pagocxp,monto_pagocxp,usuario) VALUES ('$foliocxp','$fechavp','$referenciavp','$observacionesvp','$metodovp','$montovp','$usuario')";
        $resultado = $conexion->prepare($consulta);
