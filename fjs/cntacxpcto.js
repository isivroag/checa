$(document).ready(function () {
    ...
    var textcolumnas2=permisos2();
        ...
        document.getElementById('importecto').onblur = function () {
      ...
      $(document).on("change", "#partidacto", function () {
        fsubpartida($(this).val());
      });
    
      function fsubpartida(partida) {
       
        if (partida == 1) {
          $("#fgrupo").css("display", "block");
          $("#subpartidacto").empty();
          $("#subpartidacto").val(0);
          $.ajax({
            type: "POST",
            url: "bd/cargarsubpartidacto.php",
            dataType: "json",
            async: false,
            data: {
              partida: partida,
  ...
  // PERMISOS TABLA PRINCIPAL
    ...
    //TABLA VER COSTO
    tablaVercto = $('#tablaVercto').DataTable({
      paging:false,
      searching:false,
      ordering:false,
      info:false,
      ...
      columnDefs: [
          {
              targets: -1,
              data: null,
              defaultContent:"<div class='text-center'><div class='btn-group'><button class='btn btn-sm btn-danger btnBorrarcto'  data-toggle='tooltip' data-placement='top' title='Borrar'><i class='fa-solid fa-trash'></i></button>\
              </div></div>",
            },
        {
          targets: 5,
      ...
      if (
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data == 1) {
              facturaexitosa()
              window.location.href = 'cntacxpcto.php'
            } else {
              facturaerror()
            }
          },
    ...
    $(document).on('click', '.btnBorrarcto', function () {
      folio = $('#foliocxpcto').val()
      fila = $(this).closest('tr')
    ...
    $(document).on('click', '#btnAgregarcto', function () {
      ...
      partida=$('#partidacto').val()
      subpartida = $("#subpartidacto").val();
      monto = $('#importecto').val().replace(/,/g, '')
      costoact=$('#sumacto').val()
      totalcxp=$('#montocxp').val()
      opcion=1

      importecto=parseFloat(monto)+parseFloat(costoact)
      ...
      if (
        ...
        if (parseFloat(importecto) > parseFloat(totalcxp)) {
          swal.fire({
            title: 'Importe Excedido',
            text:
              'La suma del monto capturado y los costos ya definidos exceden el total de la Cxp',
            icon: 'warning',
            focusConfirm: true,
            confirmButtonText: 'Aceptar',
          })
    ...
    function vercosto(folio){
      tablaVercto.clear()
      tablaVercto.draw()
      
      $.ajax({
        type: 'POST',
        url: 'bd/vercxpcto.php',
        dataType: 'json',
        async:false,
        data: { folio: folio },
          ...
          for (var i = 0; i < res.length; i++) {
            tablaVercto.row
              .add([
                ...
                res[i].id_subpartidacto,
                res[i].nom_subpartidacto,
                res[i].monto,
              ])
              .draw()
              sumacxp+= parseFloat(res[i].monto)
          }
          $('#sumacto').val(sumacxp)
        },
      })
    ...
    $(document).on('click', 'btnEditar', function () {
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          id_subpartidacto=res[0].id_subpartidacto
          ...
          fsubpartida(id_partidacto);
          $('#subpartidacto').val(id_subpartidacto)
          $('#modalReq').modal('show')
        },
      })
